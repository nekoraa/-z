...
#
#         目标数据 = torch.randint(1, 5, (9,))  # 生成[1, 4]之间的随机整数，长度9
#         目标数据 = torch.cat((目标数据, torch.tensor([5])))  # 将最后一个元素设置为5，得到长度为10的张量
#         目标数据 = torch.cat((torch.tensor([0]), 目标数据[1:]))
#         # 生成第二个张量 张量2
#         # 将张量1扩展为[4, 129, 10]，每个通道的数据都是张量1
#         源数据 = 目标数据.unsqueeze(0).unsqueeze(0).expand(4, 129, 10)
#
#         return 源数据.float(), 目标数据.to(torch.int64)
